...
- [ ] 4.1 Implementar el fix únicamente en el área que indicó 3.5; no presuponer causa ni empezar antes de tener esa clasificación. (Spec: Corrección de Fase B guiada por la tabla de decisión). Verificable: local (compilación) + CI (comportamiento).
- [ ] 4.2 Si la fila aplicable es "timestamp anterior a la falla", ajustar solo cómo y cuándo observa el test (p. ej. `CamundaAssert.setAssertionTimeout`), mismos ids y severidad, sin tocar el BPMN. (Spec: Corrección de Fase B guiada por la tabla de decisión). Verificable: local + CI.
- [ ] 4.3 Si la fila aplicable es incidente técnico en `disburse-loan`, ampliar el `catch` de `DisbursementWorker` aquí; si no aplica, este ítem queda diferido a un cambio de seguimiento propio y no se ejecuta. (Spec: Prohibiciones / defecto diferido). Verificable: local + CI.
- [ ] 4.4 Ejecutar la compuerta de 2.1 antes de cualquier push del fix. Verificable: 100% local.
- [ ] 4.5 Abrir PR2 (base: main tras merge de PR1) y disparar el run de CI. (Spec: Cierre de Fase B con evidencia real de CI). Verificable: CI-only.
- [ ] 4.6 Confirmar cierre válido: 58/58, `conclusion=success`, registrar URL; una inferencia local no se acepta como evidencia. (Spec: Cierre de Fase B con evidencia real de CI). Verificable: CI-only.
- [ ] 4.7 Confirmar que ningún commit de A o B introdujo `continue-on-error`, `-Dtest=`, `@Disabled` ni aserciones debilitadas. (Spec: Prohibiciones sobre cómo se alcanza el verde). Verificable: local por inspección del diff.
